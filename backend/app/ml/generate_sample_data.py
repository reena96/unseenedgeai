...
def generate_sample_data(n_samples=1000):
    ...
    data = {"student_id": [f"student_{i:04d}" for i in range(1, n_samples + 1)]}

    # 1. Linguistic Features (16 columns)
    # Skill-specific markers (0.0 - 1.0)
    data["empathy_markers"] = np.random.beta(
        2, 5, n_samples
    ...
    data["unique_word_count"] = (
        data["word_count"] * np.random.uniform(0.4, 0.7, n_samples)
    ).astype(int)
    data["noun_count"] = (
        data["word_count"] * np.random.uniform(0.15, 0.25, n_samples)
    ).astype(int)
    data["verb_count"] = (
        data["word_count"] * np.random.uniform(0.12, 0.22, n_samples)
    ).astype(int)
    data["adj_count"] = (
        data["word_count"] * np.random.uniform(0.08, 0.15, n_samples)
    ).astype(int)
    data["adv_count"] = (
        data["word_count"] * np.random.uniform(0.05, 0.12, n_samples)
    ).astype(int)

    # 2. Behavioral Features (9 columns)
    data["task_completion_rate"] = np.random.beta(
        8, 2, n_samples
    ...
    # 3. Derived Features (7 columns - 1 per skill)
    data["empathy_social_interaction"] = (
        data["empathy_markers"] * data["social_processes"]
    )
    data["adaptability_flexibility"] = data["task_completion_rate"] * (
        1 - data["distraction_resistance"]
    )
    data["problem_solving_cognitive"] = (
        data["problem_solving_language"] * data["cognitive_processes"]
    )
    data["self_regulation_focus"] = (
        data["distraction_resistance"] * data["focus_duration"]
    )
    data["resilience_recovery"] = data["retry_count"] * data["recovery_rate"]
    data["communication_expression"] = (
        data["positive_sentiment"] * data["unique_word_count"] / 1000
    )
    data["collaboration_teamwork"] = (
        data["collaboration_indicators"] * data["social_processes"]
    )

    # 4. Target Labels (7 columns)
    ...
    # Adaptability score: influenced by task completion, time efficiency, recovery rate
    adaptability_base = (
        0.3 * data["task_completion_rate"]
        + 0.25 * (data["time_efficiency"] / 2.5)  # Normalize
        + 0.2 * data["recovery_rate"]
        + 0.1 * (1 - data["distraction_resistance"])  # Less rigid = more adaptable
        + 0.15 * np.random.random(n_samples)
    )
    data["adaptability_score"] = adaptability_base.clip(0, 1)
    ...
    # Communication score: influenced by positive sentiment, word diversity, syntactic complexity
    communication_base = (
        0.3 * (data["positive_sentiment"] + 1) / 2  # Normalize to 0-1
        + 0.25 * (data["unique_word_count"] / data["word_count"])  # Word diversity
        + 0.2 * (data["syntactic_complexity"] / 8)  # Normalize
        + 0.1 * (data["readability_score"] / 100)
        + 0.15 * np.random.random(n_samples)
    )
    data["communication_score"] = communication_base.clip(0, 1)

    # Collaboration score: influenced by collaboration indicators, social processes, leadership
    collaboration_base = (
        0.35 * (data["collaboration_indicators"] / 30)  # Normalize
        + 0.25 * data["social_processes"]
        + 0.15 * (data["leadership_indicators"] / 20)  # Normalize
        + 0.1 * data["empathy_markers"]
        + 0.15 * np.random.random(n_samples)
    )
    data["collaboration_score"] = collaboration_base.clip(0, 1)
...
def main():
    ...
    print(f"Features: {len(df.columns) - 8} (excluding student_id and 7 labels)")
    print("\nColumn names:")
