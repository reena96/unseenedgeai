...
EVIDENCE_TEMPLATES = {
    'empathy': [
        "Student demonstrated empathy by acknowledging another's perspective during group discussion",
...
REASONING_TEMPLATES = {
    'empathy': "Student shows consistent ability to recognize and respond to others' emotional states. They demonstrate understanding of different perspectives and respond with appropriate emotional support.",
    'adaptability': "Student exhibits strong adaptability when facing new situations or changes in game mechanics. They quickly adjust strategies and show flexibility in their approach to challenges.",
    'problem_solving': "Student demonstrates systematic problem-solving skills, breaking down complex challenges and exploring multiple solution pathways before selecting an approach.",
    'self_regulation': "Student shows developing self-regulation skills, with ability to monitor and manage their own emotional responses during challenging tasks.",
    'resilience': "Student displays resilience in the face of setbacks, maintaining persistence and positive attitude through multiple challenges.",
    'communication': "Student communicates effectively with peers, using clear language and active listening skills to facilitate understanding.",
    'collaboration': "Student works well in team settings, sharing resources and coordinating with others to achieve common goals."
}
...
def get_missing_skills_per_student(conn) -> dict:
    ...
    with conn.cursor() as cur:
        ...
        for student_id, first_name, last_name in students:
            ...
            needed_skills = [skill for skill in ALL_SKILLS if skill not in existing_skills]
...
def create_assessment_with_evidence(conn, student_id: str, skill_type: str) -> Tuple[str, List[str]]:
    ...
    # Get reasoning
    reasoning = REASONING_TEMPLATES.get(skill_type, f"Student demonstrates {skill_type} skills.")
    ...
    with conn.cursor() as cur:
        ...
        """, (assessment_id, student_id, skill_type, score, confidence, reasoning, now, now))
        ...
        execute_values(cur, """
            ...
            (id, assessment_id, evidence_type, source, content, relevance_score, created_at, updated_at)
            VALUES %s
        """, evidence_data)
...
def add_missing_skills(conn, missing_skills: dict):
    ...
    for student_id, data in missing_skills.items():
        ...
        for skill_type in skills_to_add:
            assessment_id, evidence_ids = create_assessment_with_evidence(conn, student_id, skill_type)
            total_assessments_created += 1
            total_evidence_created += len(evidence_ids)
            print(f"    ✓ Created {skill_type} assessment ({len(evidence_ids)} evidence items)")
...
def main():
    ...
    try:
        ...
        current_state = check_current_state(conn)
