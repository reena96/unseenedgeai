...
def main():
    ...
    try:
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            print(
                f"{'Skill':<20} {'Unique Students':<18} "
                f"{'Total Assessments':<20} {'Avg Score':<12} "
                f"{'Min':<8} {'Max':<8}"
            )
            print("-" * 100)
            for row in skills:
                print(
                    f"{row['skill_type']:<20} {row['unique_students']:<18} "
                    f"{row['total_assessments']:<20} "
                    f"{row['avg_score']:<12} {row['min_score']:<8} "
                    f"{row['max_score']:<8}"
                )
            ...
            cur.execute(
                ...
                WITH latest_assessments AS (
                    SELECT
                        ...
                        sa.created_at,
                        ROW_NUMBER() OVER (
                            PARTITION BY sa.student_id, sa.skill_type
            ...
            print(
                f"{'Skill':<20} {'Students with Latest':<22} "
                f"{'Avg Latest Score':<20} {'Avg Confidence':<15}"
            )
            print("-" * 100)
            for row in latest_stats:
                print(
                    f"{row['skill_type']:<20} "
                    f"{row['students_with_latest']:<22} "
                    f"{row['avg_latest_score']:<20} "
                    f"{row['avg_confidence']:<15}"
                )
            ...
            cur.execute(
                ...
                LEFT JOIN latest_assessments la
                    ON s.id = la.student_id AND la.rn = 1
            ...
            if incomplete_students:
                print(
                    f"⚠️  WARNING: {len(incomplete_students)} students have "
                    f"incomplete skill sets:"
                )
            ...
            cur.execute(
                ...
                SELECT
                    ...
                    (SELECT COUNT(*)
                     FROM evidence e
                     WHERE e.assessment_id = la.id) as evidence_count
                FROM students s
            ...
            if sample:
                ...
                print(
                    f"{'Skill':<20} {'Score':<10} {'Confidence':<12} "
                    f"{'Evidence':<10} {'Created At':<30}"
                )
                print("-" * 100)
                for row in sample:
                    print(
                        f"{row['skill_type']:<20} {row['score']:<10.3f} "
                        f"{row['confidence']:<12.3f} "
            ...
            print("Or to get latest assessments for each skill:\n")
            for skill in ALL_SKILLS:
                skill_lower = skill.lower()
                print(
                    f"  GET /api/v1/assessments/{sample_student_id}/"
                    f"{skill_lower}/latest"
                )
            ...
            if not incomplete_students:
                ...
                print(
                    f"  - {total_assessments / total_students:.1f} average "
                    f"assessments per student"
                )
                print("\n🔍 Next Steps:")
                print("  1. Verify the frontend is calling the correct " "API endpoint")
                print(
                    "  2. Check that the API is using the /latest endpoint "
                    "to get most recent assessments"
                )
                ...
                print(f"  - {len(incomplete_students)} students are missing " f"skills")
                print("\n🔧 Recommended Actions:")
